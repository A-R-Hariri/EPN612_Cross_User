...
for rep in REPS:
    ...
    for _loss, l_name in [
        ...
        NAME = f'within_mlp_{TAG}_{l_name}{"-ft" if FT else ""}-{rep}'
        results = []
        ...
        for d, r in enumerate(ranges):
            for i in range(*r):
                ...
                train_windows   = extract_full(train_windows, feat_list, feat_dic)
                val_windows   = extract_full(val_windows,   feat_list, feat_dic)
                test_windows   = extract_full(test_windows,  feat_list, feat_dic)
                n_feat_full = train_windows.shape[-1]  # F per sub-window
