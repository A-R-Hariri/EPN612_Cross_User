...
test_loader = create_loader(test_windows_raw, test_meta_raw['classes'], 
    batch=BATCH_SIZE, shuffle=False)

base_ids = train_meta['subjects']

for i in range(0, 1):
    fname = f"{CHECKPOINT_PATH}/cnn_within_raw/results-15.npy"
    ...
    for s in [1, 2, 4, 8, 16, 32, 64, 128, 196, 306]:
        for r in [
            ...
            _BATCH_SIZE = min(len(X) // 500, BATCH_SIZE)
