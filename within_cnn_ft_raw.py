...
for rep in REPS:
    ...
    # Within Eq
    # NAME = f'cnn_raw_within_ft_eq_{rep}'
    # results = []
    ...
    #         model = CNN()
    #         model.load_state_dict(torch.load(join(CHECKPOINT_PATH, "cnn_raw_eq", "cnn_raw_eq.pt")))
    #         weights = torch.tensor(compute_class_weight('balanced', 
    #                                     classes=np.arange(CLASSES), 
    #                                         y=train_meta['classes']),
    #                                         dtype=torch.float32,
    ...
    # Within CVaR
    NAME = f'cnn_raw_within_cvar_ft_{rep}'
    results = []
    ...
    for d, r in enumerate(ranges):
        for i in range(*r):
            ...
            model = CNN()
            model.load_state_dict(torch.load(join(CHECKPOINT_PATH, "cnn_raw_cvar", "cnn_raw_cvar.pt")))
            weights = torch.tensor(compute_class_weight('balanced', 
                                        classes=np.arange(CLASSES), 
                                            y=train_meta['classes']),
                                            dtype=torch.float32,
    ...
    # Within Rest
    NAME = f'cnn_raw_within_rest_ft_{rep}'
    results = []
    ...
    for d, r in enumerate(ranges):
        for i in range(*r):
            ...
            model = CNN()
            model.load_state_dict(torch.load(join(CHECKPOINT_PATH, "cnn_raw_rest", "cnn_raw_rest.pt")))
            weights = torch.tensor(compute_class_weight('balanced', 
                                        classes=np.arange(CLASSES), 
                                            y=train_meta['classes']),
                                            dtype=torch.float32,
